...
The pattern_label is a normalized semantic label for this request pattern. Use dot notation: "architecture.pipeline", "architecture.zones", "compliance.data_residency", "general.greeting". Inputs that ask the same KIND of question should share the same label, even if the wording differs. Keep labels stable and reusable.
